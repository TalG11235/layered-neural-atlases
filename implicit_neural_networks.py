...
class IMLP(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            if i == num_layers - 1:
                # last layer
                self.hidden.append(nn.Sequential(nn.Linear(input_dims, output_dim, bias=True)))
            else:
                self.hidden.append(nn.Sequential(nn.Linear(input_dims, hidden_dim, bias=True)))
    ...
    def quantize(self):
        for layer in self.hidden:
            if layer[0].out_features == 1:
                continue
            q = quanto.qint8
            quanto.quantize(layer, weights=q, bias=q)
            quanto.freeze(layer)
