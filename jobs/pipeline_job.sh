...
latest_experiment=$(find "$EXPERIMENTS_RESULTS_DIR" -mindepth 1 -maxdepth 1 -type d | sort -V | tail -n 1)
latest_experiment_evaluation=$(find "$latest_experiment" -mindepth 1 -maxdepth 1 -type d | sort -V | tail -n 1)
echo $latest_experiment_evaluation
...
echo $CHECKPOINT_DIR####################################
echo $latest_experiment/checkpoint####################################
...
final_output_folder=$EXPERIMENTS_DIR/outputs/$(basename "$exp_dir")
