...
##############################################################################
#  create-missing Mamba/Conda environments from all *.yml / *.yaml files found in
#  a directory.  File name (minus extension) = env name.
#
#  usage:  
#   ./scripts/setup_envs.sh  # looks in ../envs by default
...
# spec folder: 1st CLI arg **or** "../envs" relative to the script
SPEC_DIR="${1:-$SCRIPT_DIR/../envs}"
...
env_exists () {
  $CMD env list | awk '{print $1}' | grep -qx "$1"
}
...
for spec in "$SPEC_DIR"/*.yml "$SPEC_DIR"/*.yaml; do
  ...
  if env_exists "$env_name"; then
    echo "✓ '$CMD' env '$env_name' already exists – skipping."
  else
    echo "Creating env '$env_name' from '$spec' ..."
    $CMD env create -n "$env_name" -f "$spec"
  fi
done
